...
- **Deferred (per user):** Facebook-vs-Pinterest differentiation, approved-brief memory promotion, and removal of the temporary `[DEV-VALIDATION]` dev logging blocks in `ImageStudio.jsx` / `VideoStudio.jsx` / `MarketingStudio.jsx` (still present, marked for removal).

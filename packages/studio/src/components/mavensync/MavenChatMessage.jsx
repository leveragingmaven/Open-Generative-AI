 ...
 */
export function MavenChatMessage({ message, onActionClick, renderContent }) {
      ...
      <div className={`space-y-2 max-w-[85%] ${isAI ? "text-left" : "text-right"}`}>
        ...
        >
          ...
          {message.attachments && message.attachments.length > 0 && (
            <div className="mt-3 space-y-1.5 pt-2 border-t border-[#252B3B]">
              {message.attachments.map((att) => (
                <div
                  key={att.id}
                  className="inline-flex items-center gap-2 px-2.5 py-1 rounded-lg bg-[#0A0C10]/60 border border-[#252B3B] text-xs text-[#94A3B8]"
                >
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {message.actions.map((act) => (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#12151E] hover:bg-[#1A1E2B] border border-[#F3BA4A]/30 hover:border-[#F3BA4A] text-xs font-medium text-[#F3BA4A] transition-all cursor-pointer shadow-xs"
              >

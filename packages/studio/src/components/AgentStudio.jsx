    ...
    <div className="group relative aspect-[4/5] rounded-xl cursor-pointer">
      ...
      {onAdd && (
        <button
          ...
          className="absolute bottom-3 right-3 w-8 h-8 rounded-full bg-black/60 border border-white/10 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-all hover:bg-[#22d3ee] hover:text-black hover:scale-110 z-10"
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    >
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/5 mt-auto text-[10px] text-white/30 font-medium">
        ...
        <span className="flex items-center gap-2">
          {conv.twinName && <span className="text-[#22d3ee]/70">· as {conv.twinName}</span>}
...
function SkillChips({ ids }) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {names.map((name) => (
        <span key={name} className="rounded-full border border-[#22d3ee]/20 bg-[#22d3ee]/[0.06] px-2 py-0.5 text-[10px] text-[#22d3ee]/80">
          {name}
        </span>
      ))}
    </div>
...
export default function AgentStudio({ apiKey, isHeaderVisible, onToggleHeader }) {
  ...
  const templates = useMemo(() => listFeaturedAgentTemplates(), []);
        ...
        <div className="flex-shrink-0 h-16 border-b border-white/5 flex items-center justify-between px-8 bg-black/40">
          <div className="flex items-center gap-4 min-w-0">
            <button
              ...
              className="text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-[#22d3ee] transition-colors flex items-center gap-1"
            >
            ...
            <div className="min-w-0">
              <h2 className="text-sm font-black uppercase tracking-[0.15em] text-[#22d3ee] truncate">{openAgent.name}</h2>
              <p className="text-[10px] text-white/40 truncate">{openAgent.specialty}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {twin && (
              <span className="rounded-full border border-[#22d3ee]/25 bg-[#22d3ee]/[0.06] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[#22d3ee]/80">
          ...
          {!openConversation ? (
            <div className="h-full flex flex-col items-center justify-center gap-4 text-center">
              ...
              <div className="max-w-md">
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-[#22d3ee]">{openAgent.category}</p>
              ...
              <button
                onClick={startChat}
                disabled={!twinId}
                className="mt-2 px-6 py-2 bg-[#22d3ee] text-black text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-[#ebff66] transition-all active:scale-95 disabled:opacity-40"
              >
                Start conversation
              </button>
              {!twinId && (
                ...
                {getAgentMessages(openConversation.id).map((msg, i) => {
                  ...
                  return (
                    <div key={msg.id || i} className={`flex ${isAssistant ? "justify-start" : "justify-end"}`}>
                      <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${isAssistant ? "border border-white/10 bg-[#0d0d0d] text-white/85" : "bg-[#22d3ee]/15 text-[#22d3ee]"}`}>
                        {isAssistant && (
                          <p className="mb-1 text-[10px] font-black uppercase tracking-widest text-[#22d3ee]/70">
                            {openAgent.name}
                          </p>
                        )}
          ...
          <div className="flex-shrink-0 border-t border-white/5 p-4 bg-black/30">
            <div className="max-w-3xl mx-auto flex items-end gap-2">
              <textarea
                ...
                className="flex-1 resize-none rounded-xl border border-white/10 bg-[#0d0d0d] px-4 py-2.5 text-sm text-white outline-none focus:border-[#22d3ee]/50"
              />
              <button
                onClick={sendMessage}
                disabled={!chatDraft.trim()}
                className="px-5 py-2.5 bg-[#22d3ee] text-black text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-[#ebff66] transition-all active:scale-95 disabled:opacity-40"
              >
                Send
              </button>
            </div>
        ...
        <div className="flex items-center gap-6 h-full min-w-0">
          ...
          <div className="relative hidden md:block min-w-0 flex-1 max-w-xs">
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 pl-8 text-xs text-white outline-none focus:border-[#22d3ee]/40"
            />
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="hidden lg:flex items-center gap-2">
            ...
            <select
              ...
              className="rounded-lg border border-white/10 bg-[#0d0d0d] px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-widest text-[#22d3ee] outline-none focus:border-[#22d3ee]/40"
            >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
        {activeMainTab === "my-chats" ? (
          visibleChats.length === 0 ? (
            <EmptyState title="No chats yet" action={<button onClick={() => setActiveMainTab("featured")} className="text-[10px] text-[#22d3ee] hover:text-white border border-[#22d3ee]/20 hover:border-white/20 px-4 py-2 rounded-lg transition-colors">Browse Agents</button>} />
          ) : (
...
const inputCls =
  "w-full rounded-lg border border-white/10 bg-[#0d0d0d] px-3 py-2 text-sm text-white outline-none focus:border-[#22d3ee]/50";
        ...
        <div className="mt-5">
          ...
          <div className="mt-5 flex justify-end gap-2">
            ...
            <button
              ...
              className="px-5 py-2 bg-[#22d3ee] text-black text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-[#ebff66] transition-all disabled:opacity-40"
            >
              Generate Profile
            </button>
          </div>
        ...
        <div className="mt-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <Field label="Category">
              <div className="rounded-lg border border-white/10 bg-[#0d0d0d] px-3 py-2 text-sm text-[#22d3ee]">{draftProfile.category}</div>
            </Field>
            <Field label="Avatar">
              <div className="flex items-center gap-2">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              ...
              className="px-5 py-2 bg-[#22d3ee] text-black text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-[#ebff66] transition-all"
            >
              Save Agent
            </button>
          </div>
